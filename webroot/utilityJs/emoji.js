...
function buildEmoji(text) {
    ...
    for (var j = 0, len = matches.length; j < len; ++j) {
        if (emoji[matches[j]]) {
            //todo: change to oss url
            text = text.replace(matches[j], '<img class="emoji" src="' + window.imageRoot + '/images/emoji/' + emoji[matches[j]].file + '" />');
        }
    }
    return text;
}
...
CEmojiEngine.prototype.__renderChangeCol = function () {
    for (var i in this._emojiList) {
        ...
        for (var key in emojiList) {
            ...
            var img = new Image();
            //todo: change to oss url?
            img.src = window.imageRoot + "/images/" + emojiList[key]['file'];
    ...
    for (var i in this._pinupList) {
        ...
        span.id = 'chn-pinup-' + i;
        var img = new Image();
        img.src = window.imageRoot + "/images/" + pinupList[0];
        span.appendChild(img);
        this._changeColumn.appendChild(span);
    }
}
