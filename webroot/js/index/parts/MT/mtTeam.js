 ...
 * @return {void}
 */
MT.buildTeams = function (id) {
    this.ui.initSessionList();
	// var data = {
	// 	teams: cache.getTeamlist()
	// }
	// if (!this.teams) {
	// 	var options = {
	// 		data: data,
	...
	// 	// this.teams.inject($('#teams').get(0))
     ...
     //        console.log("buildTeams", options);
     //    }
	// 	//todo build team in address book and a new session in session list
	// 	// cache.setSessions(options.data.sessions);
	// 	this.ui.initSessionList();
    // } else {
 ...
 * 主动去拿群列表
 */
MT.getTeamMembers = function (id, callback, alertID) {
	var that = this
	this.mysdk.getTeamMembers( id, function (  err, obj ) {
		if (!err) {
			cache.setTeamMembers( id, obj )
			callback();
		} else {
			that.ui.showAlert(err.message)
		}
	})
}
/**
 ...
 * demo里跟点击列表处理一致了
 */
MT.clickTeamAvatar = function (account, type) {
 ...
 * @return {void}
 */
MT.showTeamView = function (type, teamType) {
	var that = this
	this.teamType = teamType
	var list = cache.getFriendslistOnShow()
...
MT.createTeam = function (list) {
	...
	// 创建普通群
	var owner = cache.getUserById(userUID).nick
...
MT.cbDoTeamSearch = function (err, data) {
	if (err) {
		alert(err.message)
	} else {
 ...
 * 群资料
********************************************************************/
MT.showTeamInfo = function () {
	var that = this
	this.$teamInfoContainer.load('./teamInfo.html', function () {
		// 获取群成员
		var teamId = that.crtSessionAccount,
			teamInfo = cache.getTeamById(teamId)
		if (!teamInfo) {
			return
		}
		...
		var showRole = function () {
			myTeamInfo = cache.getTeamMemberInfo(userUID, teamId)
			if (type === "advanced") {
				//其本信息
		...
		var showMember = function () {
			...
			for (var i = 0; i < members.length; i++) {
				if (!cache.getUserById(members[i].account)) {
		...
		var showUI = function () {
			...
			for (var i = 0, l = members.length; i < l; ++i) {
				var member = members[i],
					...
					avatar = getAvatar(cache.getUserById(account) ? cache.getUserById(account).avatar : ""),
...
MT.sortTeamMembers = function (members) {
...
MT.removeTeamMember = function (event) {
...
MT.setBeInviteMode = function (evt) {
...
MT.setUpdateTeamMode = function (evt) {
